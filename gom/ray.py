...
class Ray():
    def __init__(self, pose, max_coords=600, max_segments=100):
    ...
    def trace_ray(self, assembly):
        ...
        for _ in range(self.max_segments):
            ...
            for element in assembly:
                ...
                ray_segment = self.parametric_form(ray_path[-1][0], ray_path[-1][1], ray_path[-1][2])

                # Segment the element (pose, width) into parametric form (x,y,dx,dy)
                element_segment = self.create_segment(element.pose[0], element.pose[1], element.pose[2], element.size[0])
                ...
                if self.find_intersection(ray_segment, element_segment):
                    ...
                    new_ray = element.trace_ray(ray_path[-1])
                    # Add the new ray to the ray_path
                    ray_path.append(new_ray)
    ...
    def find_intersection(ray, segment):
        """
